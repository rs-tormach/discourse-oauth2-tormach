...
# about: Allows users to login to a forum from an external OAuth provider
# url: https://github.com/rs-tormach/discourse-oauth2-tormach

enabled_site_setting :torauth2_enabled

class ::OmniAuth::Strategies::Oauth2Tormach < ::OmniAuth::Strategies::OAuth2
...
class TorOAuth2FaradayFormatter < Faraday::Logging::Formatter
  def request(env)
    warn <<~LOG
  ...
  def response(env)
    warn <<~LOG
      Tormach OAuth2 Debugging: response status #{env.status}
...
#
# Example usage:
# DiscoursePluginRegistry.register_oauth2_tormach_required_json_path({
#   path: "extra:data.is_allowed_user",
#   required_value: true,
#   error_message: I18n.t("auth.user_not_allowed")
# }, self)
DiscoursePluginRegistry.define_filtered_register :oauth2_tormach_required_json_paths

class ::TorOAuth2BasicAuthenticator < Auth::ManagedAuthenticator
  def name
    "oauth2_tormach"
  end
  ...
  def register_middleware(omniauth)
    omniauth.provider :oauth2_tormach,
                      name: name,
                      setup:
                        lambda { |env|
  ...
  def json_walk(result, user_json, prop, custom_path: nil)
    path = custom_path || SiteSetting.public_send("torauth2_json_#{prop}_path")
    if path.present?
  ...
  def fetch_user_details(token, id)
    ...
    if user_json_response.status == 200
      ...
      if user_json.present?
        ...
        DiscoursePluginRegistry.oauth2_tormach_additional_json_paths.each do |detail|
          prop = "extra:#{detail}"
  ...
  def after_authenticate(auth, existing_account: nil)
    ...
    if SiteSetting.torauth2_fetch_user_details? && SiteSetting.torauth2_user_json_url.present?
      if fetched_user_details = fetch_user_details(auth["credentials"]["token"], auth["uid"])
        ...
        DiscoursePluginRegistry.oauth2_tormach_required_json_paths.each do |x|
        ...
        result = Auth::Result.new
        result.failed = true
        result.failed_reason = I18n.t("login.tor_authenticator_error_fetch_user_details")
        return result
      end
    end
...
auth_provider title_setting: "torauth2_button_title", authenticator: TorOAuth2BasicAuthenticator.new
...
load File.expand_path(
       "../lib/validators/oauth2_tormach/tor_oauth2_fetch_user_details_validator.rb",
       __FILE__,
     )
